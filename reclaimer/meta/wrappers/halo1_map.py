...
class Halo1Map(HaloMap):
    ...
    def meta_to_tag_data(self, meta, tag_cls, tag_index_ref, **kwargs):
        ...
        if tag_cls == "actv":
            ...
            # reduce z offset of detail objects by global offset values
            for sbsp_dobs in meta.detail_objects.STEPTREE:
                dobc_palette = []
                try:
                    scnr_meta = self.get_meta(self.tag_index.scenario_tag_id)
                    scnr_meta and dobc_palette.extend(
                        scnr_meta.detail_object_collection_palette.STEPTREE
                        )
                except Exception:
                    print(format_exc())
                    print("Unable to correct detail object global z-offsets.")

                cells    = sbsp_dobs.cells.STEPTREE
                ref_vecs = sbsp_dobs.z_reference_vectors.STEPTREE
                for i, pal in enumerate(dobc_palette):
                    z_off = 0
                    try:
                        z_off = self.get_meta(pal.name.id).global_z_offset/8
                    except Exception:
                        print(format_exc())
                        print("Unable to read detail_object_collection %d" %
                              pal.name.id)

                    if not z_off:
                        continue

                    for cell in (c for c in cells if ((c.layer_flags>>i)&1)):
                        ref_vec_idx = cell.count_index + i
                        if ref_vec_idx in range(len(ref_vecs)):
                            ref_vecs[ref_vec_idx].d -= z_off
