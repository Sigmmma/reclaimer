...
class JmsNode:
    ...
    @property
    def checksum(self):
        # NOTE: This correctly calculates the crc ONLY when this
        #       is the ONLY node in the hierarchy. There is more
        #       that goes into combining them, and may include
        #       bitwise operations and the child/sibling indices
        return sum(v<<(len(self.name)-i) for i, v in
                   enumerate(self.name.encode('latin-1'), 1))
        #* (n.first_child + n.sibling_index + 2)
