...
from reclaimer.resources.P8_Palette import *
...
def Convert_Bitmap_Tag(tag, **kwargs):
    ...
    tagpath = kwargs.get("tagpath",tag.tagpath)
...
def Parse_Bitmap_Blocks(tag):
    ...
    for i in range(tag.Bitmap_Count()):
        ...
        Tex_Infos.append({"Width":Max_Width, "Height":Max_Height, "Depth":Max_Depth,
                          ...
                          "Filepath":splitext(tag.tagpath.replace(tagsdir,datadir))[0]})
...
def Bitmap_Sanitize(tag):
    ...
    tag.tagdata.Data.Compressed_Color_Plate_Data.Data = bytearray()
