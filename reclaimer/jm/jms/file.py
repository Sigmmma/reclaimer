...
def read_jms(jms_stream, stop_at="", perm_name=None):
    '''
    Converts a jms data string/stream into a JmsModel instance.
    '''
    jm_iter = jm_reader.JMReader(jms_stream)
    version = jm_iter.next_int

    if (version < const.JMS_VER_HALO_1_OLDEST_KNOWN or 
        version > const.JMS_VER_HALO_2_RETAIL):
        print("Unknown JMS version '%s'" % version)
        return None

    if version <= const.JMS_VER_HALO_1_RETAIL:
        # Halo 1
        return _read_jms_8200(jm_iter, stop_at, perm_name, version)

    # try again, but with comments
    jm_iter.comments = True
    jm_iter.reset_stream()
    version = jm_iter.next_int

    if version <= const.JMS_VER_HALO_2_RETAIL:
        # Halo 2
        return _read_jms_8210(jm_iter, stop_at, version)


def _read_jms_8200(jm_iter, stop_at="", perm_name=None, version=None):
    ...
    # Halo 1
    try:
        jms_model.node_list_checksum = jm_iter.next_int
    except Exception:
    ...
    if not stop:
        ...
        try:
            jms_model.nodes = []
            for i in range(jm_iter.next_int):
                name        = jm_iter.next
                first_child = jm_iter.next_int
                sibling     = jm_iter.next_int
                quat_ijkw   = [jm_iter.next_float for _ in range(4)]
                pos_xyz     = [jm_iter.next_float for _ in range(3)]
        ...
        try:
            ...
            for i in range(jm_iter.next_int):
                jms_model.materials.append(
                    JmsMaterial(jm_iter.next, jm_iter.next)
                    )
    ...
    if not stop:
        ...
        try:
            ...
            for i in range(jm_iter.next_int):
                name        = jm_iter.next
                region      = (jm_iter.next_int
                               if jms_model.has_marker_regions else 0)
                parent      = jm_iter.next_int
                quat_ijkw   = [jm_iter.next_float for _ in range(4)]
                pos_xyz     = [jm_iter.next_float for _ in range(3)]
                radius      = (jm_iter.next_float
    ...
    if not stop:
        ...
        try:
            jms_model.regions = [
                jm_iter.next for i in range(jm_iter.next_int)
                ]
        except Exception:
            print(traceback.format_exc())
    ...
    if not stop:
        ...
        try:
            jms_model.verts = [None] * jm_iter.next_int
            for i in range(len(jms_model.verts)):
                region      = (jm_iter.next_int
                               if jms_model.has_vert_regions else 0)
                node_0      = jm_iter.next_int
                px, py, pz  = [jm_iter.next_float for _ in range(3)]
                ni, nj, nk  = [jm_iter.next_float for _ in range(3)]
                node_1      = jm_iter.next_int
                node_weight = jm_iter.next_float
                tu, tv      = [jm_iter.next_float for _ in range(2)]
                tw          = jm_iter.next_float if jms_model.has_3d_uvws else 0
    ...
    if not stop:
        ...
        i = 0 # make sure i is defined in case of exception
        try:
            jms_model.tris = [None] * jm_iter.next_int
            for i in range(len(jms_model.tris)):
                region = (jm_iter.next_int
                          if jms_model.has_face_regions else 0)
                shader = jm_iter.next_int
                v0, v1, v2 = [jm_iter.next_int for _ in range(3)]
...
def _read_jms_8210(jm_iter, stop_at="", version=const.JMS_VER_HALO_2_RETAIL):
    # Halo 2
    # NOTE: This function is incomplete. It will not fully work
    jms_models = {}

    jms_data = list(jm_iter) # i'm not reworking this to allow streaming
    dat_i = 1
