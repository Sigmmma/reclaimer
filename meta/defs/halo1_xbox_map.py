...
def tag_path_pointer(node=None, parent=None, new_value=None, **kwargs):
    ...
    if magic is None:
        magic = XBOX_INDEX_MAGIC - m_head.tag_index_offset
...
def tag_index_array_pointer(node=None, parent=None, new_value=None, **kwargs):
    ...
    magic = kwargs.get('magic')
    if magic is None:
        magic = XBOX_INDEX_MAGIC - b_parent.map_header.tag_index_offset
...
tag_index_header_xbox = Struct("tag index header",
    LUInt32("index magic", DEFAULT=XBOX_INDEX_MAGIC),
    LUInt32("base magic"),
    LUInt32("map id"),
    LUInt32("tag count"),
    ...
    SIZE=36, POINTER='.map_header.tag_index_offset'
    )
...
map_def.index_magic = XBOX_INDEX_MAGIC
